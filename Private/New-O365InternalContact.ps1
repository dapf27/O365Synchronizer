function New-O365InternalContact {
  ...
  try {
    $StatusNew = New-O365WrapperPersonalContact -UserId $UserID @PropertiesToUpdate -ParentFolderId $ContactFolderId -WhatIf:$WhatIfPreference -FileAs "$($GuidPrefix)$($User.Id)" -ErrorAction SilentlyContinue
    $ErrorMessage = ''
  } catch {
    $ErrorMessage = $_.Exception.Message
    if ($User.Mail) {
