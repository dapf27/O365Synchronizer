function Sync-O365PersonalContact {
  <#
    ...
    .PARAMETER ContactFolderName
    Name of the folder for the users to synchronize in

    .EXAMPLE
    Sync-O365PersonalContact -UserId 'przemyslaw.klys@test.pl' -Verbose -MemberTypes 'Contact', 'Member' -ContactFolderName 'name of the folder' -WhatIf
  ...
  foreach ($User in $UserId) {
    ...
    $Actions = Sync-InternalO365PersonalContact -UserId $User -ExistingUsers $ExistingUsers -ExistingContacts $ExistingContacts -MemberTypes $MemberTypes -RequireEmailAddress:$RequireEmailAddress.IsPresent -GuidPrefix $GuidPrefix -ContactFolderName $ContactFolderName -WhatIf:$WhatIfPreference
    if ($PassThru) {
      $Actions
    }
  }
